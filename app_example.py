...
input_fireworks = 'fireworks.xlsx'
output_menu = 'menu.txt'
output_receipt = 'receipt.txt'
budget = 100.00
...
def sort_fireworks(fireworks):
    return sorted(fireworks, key=lambda fw: fw['price'], reverse=True)
...
def handle_selections(fireworks, budget):
    ...
    print("Order Summary:")
    for selection in selections:
        print(f"{selection['name']} - ${selection['price']:.2f}")
    print(f"Remaining budget: ${remaining_budget:.2f}")

    return selections, remaining_budget

# Function to prompt user for color selection
...
# Function to draw fireworks using turtle
